...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    const { memeObject } = req.body;

    // Deconstruct the JSON stringified meme object
    const memeData = JSON.parse(memeObject);
    ...
    // update file path format for storage
    ...
    const db = req.db;
    const memes = db.get("memes"); // memes is the table for memes

    memes.insert(newMeme);

    // get all memes from the database
    //const memeData = await memes.find();

    // array of buffers containing meme data
    //const memeBuffers = memeData.map((meme) => {
    //  const buffer = fs.readFileSync(meme.imagePath); //notsure
    //  return [buffer, meme.name];
    //});

    // zip meme and send respond
    //const zipBuffer = await imageZipper.zipBuffers(memeBuffers, res, true);
    //res.end(zipBuffer);

    // target file size from the request body in front end
    // TODO: Update with parameter name FROM FRONT END I HAVE TO ADD IT

    //resize
    //const resizedImageBuffer = await resizeImage(imageBuffer, targetFileSize);

    //shareable URL
    //const shareableURL = `http://127.0.0.1:3001/${newMemeId}`; // TODO: UPDATE WITH OUR URL
    const shareableURL = updatedPath;

    res
      .status(201)
      .json({ message: "Meme created successfully 2", imageURL: shareableURL });
  } catch (error) {
    console.error("Error creating meme:", error);
    res.status(500).json({ error: "Internal server error" });
  }

  // Call the imageZipper function to zip the memes and send the response
  //const zipBuffer = await imageZipper.zipBuffers(memeBuffers, res, true);
  // res.end(zipBuffer);
  // } catch (error) {
  // console.error('Error creating meme:', error);
